...
ano = int(input ('Qual é o ano da eleição que você quer consultar? Temos 2008, 2012 e 2016.'))

#Faz o download e unzip dos arquivos txt dos candidatos
...
cargo_escolhido = input('Agora você precisa escolher o cargo que vamos analisar. As opções dependem do ano de eleição: Ex.: Vereador, Deputado, Prefeito e Vice-prefeito')
cargo_escolhido = cargo_escolhido.upper()
filtrado = juntar_tabelas[juntar_tabelas.Cargo == cargo_escolhido]



#Criando um filtro com o pandas para escolher a situação política analisada.
...
#Criando um filtro para cidade que você quer analisar
...
filtrado_cidade.to_csv(f'TabelaFinal - {estado_analise} - {ano} - {cargo_escolhido}.csv')
